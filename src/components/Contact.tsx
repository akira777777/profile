    ...
    <Section bgLetter="C" id="contact">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <FadeIn>
          <form
            onSubmit={onSubmit}
            className="border border-border bg-card/55 p-5 transition-colors hover:border-accent/40 sm:p-7"
            noValidate
          >
            {!hasKey ? (
            ...
            <button
              ...
              className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 bg-accent px-6 text-sm font-semibold text-accent-foreground transition-all hover:scale-[1.005] hover:bg-accent/90 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {status === "submitting" ? (
                f.sending
              ) : (
        ...
        <FadeIn delay={0.15}>
          <div className="flex h-full flex-col gap-4 border border-border bg-background/45 p-5 sm:p-7">
            <p className="text-sm font-medium text-muted">{contact.direct}</p>

            <button
              type="button"
              onClick={copyEmail}
              className="group flex items-center justify-between gap-3 border border-border bg-card/45 p-4 text-left transition-all hover:border-accent/45 hover:bg-card/75"
            >
