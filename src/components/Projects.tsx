...
function CardVisual({
  ...
  return (
    <div
      className={`relative flex min-h-[150px] items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br ${project.accent} p-6`}
    >
      <div
        aria-hidden="true"
      ...
      />
      <span className="text-5xl font-black tracking-tight text-white/95 drop-shadow-sm">
        {project.monogram}
      </span>
...
function ProjectCard({
  ...
  return (
    <article
      className={`group relative flex flex-col overflow-hidden rounded-3xl border border-border bg-background p-4 transition-all hover:border-accent/40 hover:shadow-lg hover:shadow-black/5 ${
        isFeatured ? "sm:p-6" : ""
      }`}
    >
      {isFeatured ? (
          ...
          <div className="flex flex-col justify-center gap-4 p-2">
            <div className="flex flex-wrap gap-2">
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-border bg-card px-2.5 py-1 text-xs font-medium text-muted"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex flex-1 flex-col gap-3 p-3 pt-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-border bg-card px-2 py-0.5 text-[11px] font-medium text-muted"
                >
                  {tag}
                </span>
              ))}
    ...
    <Section id="projects">
      ...
      <div className="mt-10 flex flex-col gap-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {rest.map((project) => (
            <div key={project.id} className="transition-transform hover:-translate-y-0.5">
